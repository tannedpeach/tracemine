...
async def capture(source: Path, task: str, timeout: float) -> int:
    ...
    def ignore(directory: str, names: list[str]) -> list[str]:
        return [n for n in names if n in {".git", ".venv", "node_modules", "__pycache__", ".env"}
    ...
    with (root / "events.jsonl").open("wb") as stdout, (root / "stderr.log").open("wb") as stderr:
        proc = await asyncio.create_subprocess_exec(
            ...
            stdin=asyncio.subprocess.PIPE, stdout=stdout, stderr=stderr,
    ...
    (root / "capture.json").write_text(json.dumps({"exit_code": proc.returncode,
